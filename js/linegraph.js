function drawLineChart(dataset, xName, yObjs, axisLables) {
    var chartObj = {};
    ...
    chartObj.formatAsSI = d3.format(".2s");
    chartObj.xFormatter = chartObj.formatAsNumber;
    chartObj.yFormatter = chartObj.formatAsSI;
    ...
    chartObj.yScale = d3.scale.linear().range([chartObj.height, 0]).domain([4000000000, d3.max(chartObj.yFuncts.map(chartObj.max))]);
    ...
    chartObj.yAxis = d3.svg.axis().scale(chartObj.yScale).orient("left").tickFormat(function (d) { return chartObj.formatAsSI(d).replace('G', 'B'); });
